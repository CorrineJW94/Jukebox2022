...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_FILES['menu_file'])) {
    if(!file_exists(SJ_UPLOAD_DIR))
        mkdir(SJ_UPLOAD_DIR);

    $ext = explode('.', $_FILES['menu_file']['name']);
    $ext = $ext[count($ext) -1];
    $file = SJ_UPLOAD_DIR . "/menu." . $ext;
    rename($_FILES['menu_file']['tmp_name'], $file);
    $db->setMenu(dirname($_SERVER['PHP_SELF']) . '/' . $file);
}
...
<head>
    ...
    <div class="header">
        ...
        <div class="wrapper menu">
            <p class="status">
                This is where you can upload a single file that will be displayed to the user as a menu link in the mobile interface
            </p>
            </br>
            <?php
                if($db->hasMenu()) {
                    ?>
                    <a href="<?php echo $db->getMenu();?>" target="_blank">Click here to View the current uploaded menu</a>
                    </br>
                    <?php
                }
            ?>
            ...
            <div class="wrapper fields_wrapper">
                <p class="title">Upload a new Menu</p>
                <form method="POST" enctype="multipart/form-data">
                    <input type="file" name="menu_file"/>
                    </br>
                    <input class="button" type="submit" value="Upload" name="save">
                </form>
            </div>
        </div>
    </body>
